...
const ChannelHeaderButton = () => {
    ...
    const button = (
        <CallButton
            ...
            className={
                'style--none call-button ' +
                (inCall || restricted ? 'disabled' : '')
            }
        ...
        >
            ...
            <CallButtonText>{callButtonText}</CallButtonText>
        </CallButton>
    );
            ...
            <OverlayTrigger
                ...
                overlay={
                    <Tooltip id='tooltip-limit-header'>
                        {formatMessage({
                            defaultMessage:
                                'Calls are not available in a DM with a deactivated user.',
                        })}
                    </Tooltip>
                }
            >
                    ...
                    <Tooltip id='tooltip-limit-header'>
                        <Header>
                            {formatMessage(
                                {
                                    defaultMessage:
                                        'This call is at its maximum capacity of {count, plural, =1 {# participant} other {# participants}}.',
                                },
                                { count: maxCallParticipants },
                            )}
                        </Header>
                        ...
                        {cloudStarter && isAdmin && (
                            <SubHeader>
                                {formatMessage(
                                    {
                                        defaultMessage:
                                            'Upgrade to Cloud Professional or Cloud Enterprise to enable group calls with more than {count, plural, =1 {# participant} other {# participants}}.',
                                    },
                                    { count: maxCallParticipants },
                                )}
                            </SubHeader>
                        ...
                        {isCloudPaid && (
                            <SubHeader>
                                {formatMessage(
                                    {
                                        defaultMessage:
                                            'At the moment, {count} is the participant limit for cloud calls.',
                                    },
                                    { count: maxCallParticipants },
                                )}
                            </SubHeader>
...
const CallButton = styled.button<{
    ...
    &&& {
        ${(props) =>
            ...
            `}
        cursor: ${(props) =>
            props.$restricted && props.$isCloudPaid
                ? 'not-allowed'
                : 'pointer'};
    }

    ${(props) =>
        props.$isClientConnecting &&
        css`
            &&&& {
