...
import LiveCaptionsNumThreadsPerTranscriber from 'src/components/admin_console_settings/recordings/live_captions_num_threads_per_transcriber';
import LiveCaptionsNumTranscribers from 'src/components/admin_console_settings/recordings/live_captions_num_transcribers';
...
import {
    CALL_RECORDING_POST_TYPE,
    CALL_START_POST_TYPE,
    CALL_TRANSCRIPTION_POST_TYPE,
    DisabledCallsErr,
} from 'src/constants';
...
import { handleDesktopJoinedCall } from './desktop';
...
export default class Plugin {
    ...
    private registerReconnectHandler(
        ...
        this.unsubscribers.push(() =>
            registry.unregisterReconnectHandler(handler),
        );
    }

    private registerWebSocketEvents(registry: PluginRegistry, store: Store) {
        registry.registerWebSocketEventHandler(
            `custom_${pluginId}_channel_enable_voice`,
            (ev) => {
                store.dispatch({
        ...
        registry.registerWebSocketEventHandler(
            `custom_${pluginId}_channel_disable_voice`,
            (ev) => {
                store.dispatch({
                    type: RECEIVED_CHANNEL_STATE,
        ...
        registry.registerWebSocketEventHandler(
            `custom_${pluginId}_user_unmuted`,
            (ev) => {
        ...
        registry.registerWebSocketEventHandler(
            `custom_${pluginId}_user_voice_on`,
            (ev) => {
        ...
        registry.registerWebSocketEventHandler(
            `custom_${pluginId}_user_voice_off`,
            (ev) => {
        ...
        registry.registerWebSocketEventHandler(
            `custom_${pluginId}_user_screen_on`,
            (ev) => {
        ...
        registry.registerWebSocketEventHandler(
            `custom_${pluginId}_user_screen_off`,
            (ev) => {
        ...
        registry.registerWebSocketEventHandler(
            `custom_${pluginId}_user_raise_hand`,
            (ev) => {
        ...
        registry.registerWebSocketEventHandler(
            `custom_${pluginId}_user_unraise_hand`,
            (ev) => {
        ...
        registry.registerWebSocketEventHandler(
            `custom_${pluginId}_user_reacted`,
            (ev) => {
        ...
        registry.registerWebSocketEventHandler(
            `custom_${pluginId}_call_host_changed`,
            (ev) => {
        ...
        registry.registerWebSocketEventHandler(
            `custom_${pluginId}_call_job_state`,
            (ev) => {
        ...
        registry.registerWebSocketEventHandler(
            `custom_${pluginId}_user_dismissed_notification`,
            (ev) => {
        ...
        registry.registerWebSocketEventHandler(
            `custom_${pluginId}_host_screen_off`,
            (ev) => {
        ...
        registry.registerWebSocketEventHandler(
            `custom_${pluginId}_host_lower_hand`,
            (ev) => {
        ...
        registry.registerWebSocketEventHandler(
            `custom_${pluginId}_host_removed`,
            (ev) => {
    ...
    private initialize(registry: PluginRegistry, store: Store) {
        ...
        const sidebarChannelLinkLabelComponentID =
            registry.registerSidebarChannelLinkLabelComponent(ChannelLinkLabel);
        this.unsubscribers.push(() =>
            registry.unregisterComponent(sidebarChannelLinkLabelComponentID),
        );
        ...
        registry.registerPostTypeComponent(
            CALL_TRANSCRIPTION_POST_TYPE,
            PostTypeTranscription,
        );
        registry.registerPostTypeComponent(
            'custom_cloud_trial_req',
            PostTypeCloudTrialRequest,
        );
        ...
        registry.registerDesktopNotificationHook?.(
            async (post, msgProps, channel, teamId, args) => {
        ...
        const connectToCall = async (
            ...
            if (!channelIDForCurrentCall(store.getState())) {
                ...
                if (
                    profilesInCallInChannel(store.getState(), channelId)
                        .length > 0
                ) {
                    followThread(store, channelId, teamId);
                }
            } else if (
                channelIDForCurrentCall(store.getState()) !== channelId
            ) {
        ...
        const joinCall = async (
            ...
            const explicitlyEnabled = callsExplicitlyEnabled(
                store.getState(),
                channelId,
            );
            const explicitlyDisabled = callsExplicitlyDisabled(
                store.getState(),
                channelId,
            );
            ...
            if (
                ...
                explicitlyEnabled ||
                (!explicitlyDisabled && defaultEnabled(store.getState()))
            ) {
        ...
        registry.registerAdminConsoleCustomSetting(
            'LiveCaptionsModelSize',
            LiveCaptionsModelSize,
        );
        registry.registerAdminConsoleCustomSetting(
            'LiveCaptionsNumTranscribers',
            LiveCaptionsNumTranscribers,
        );
        registry.registerAdminConsoleCustomSetting(
        ...
        if (window.desktopAPI?.onCallsError) {
            ...
            this.unsubscribers.push(
                window.desktopAPI.onCallsError(
                    (err: string, callID?: string, errMsg?: string) => {
        ...
        const connectCall = async (
            ...
            const payload = {
                ...
                channelURL: getChannelURL(
                    store.getState(),
                    getChannel(store.getState(), channelID),
                    getCurrentTeamId(store.getState()),
                ),
                rootID: rootId || '',
            };
            if (window.desktopAPI?.joinCall) {
                ...
                handleDesktopJoinedCall(
                    store,
                    await window.desktopAPI.joinCall(payload),
                );
                store.dispatch(setClientConnecting(false));
                return;
            ...
            try {
                ...
                if (needsTURNCredentials(state)) {
                    ...
                    try {
                        iceConfigs.push(
                            ...(await RestClient.fetch<RTCIceServer[]>(
                ...
                if (window.desktop) {
                    rootComponentID = registry.registerRootComponent(
                        injectIntl(ExpandedView),
                    );
                }
        ...
        const windowEventHandler = (ev: MessageEvent) => {
            ...
            if (ev.data?.type === 'connectCall') {
                ...
                ev.data?.type === 'desktop-sources-modal-request' &&
                !window.desktopAPI?.onOpenScreenShareModal
            ) {
                // DEPRECATED: legacy Desktop API logic (<= 5.6.0)
                store.dispatch(showScreenSourceModal());
            } else if (
                ev.data?.type === 'calls-joined-call' &&
                !window.desktopAPI?.joinCall
            ) {
                // DEPRECATED: legacy Desktop API logic (<= 5.6.0)
                store.dispatch(setClientConnecting(false));
                handleDesktopJoinedCall(store, ev.data.message);
            } else if (
                ev.data?.type === 'calls-join-request' &&
                !window.desktopAPI?.onJoinCallRequest
            ) {
                ...
                ev.data.message.err === 'client-error' &&
                !window.desktopAPI?.onCallsError
            ) {
                // DEPRECATED: legacy Desktop API logic (<= 5.6.0)
                store.dispatch(setClientConnecting(false));
                store.dispatch(
                    displayCallErrorModal(
                        new Error(ev.data.message.errMsg),
                        ev.data.message.callID,
                    ),
                );
                ...
                ev.data?.type === 'calls-link-click' &&
                !window.desktopAPI?.openLinkFromCalls
            ) {
                // DEPRECATED: legacy Desktop API logic (<= 5.6.0)
        ...
        const registerChannelHeaderMenuAction = () => {
            channelHeaderMenuID = registry.registerChannelHeaderMenuAction(
                ...
                async () => {
                    try {
                        const data = await RestClient.fetch<{
                            enabled: boolean;
                        }>(`${getPluginPath()}/${currChannelId}`, {
                            method: 'post',
                            body: JSON.stringify({
                                enabled: callsExplicitlyDisabled(
                                    store.getState(),
                                    currChannelId,
                                ),
                            }),
        ...
        const fetchChannels = async (
            ...
            const actions = [];
            try {
                const data = await RestClient.fetch<CallChannelState[]>(
                    `${getPluginPath()}/channels`,
                    { method: 'get' },
                );
                ...
                for (let i = 0; i < data.length; i++) {
                    ...
                    if (skipChannelID === data[i].channel_id) {
                        logDebug(
                            'skipping channel from state loading',
                            skipChannelID,
                        );
                        continue;
                    }
                    ...
                    store.dispatch(
                        loadProfilesByIdsIfMissing(
                            getUserIDsForSessions(call.sessions),
                        ),
                    );
                    ...
                    if (
                        ...
                        actions.push({
                            type: USERS_STATES,
                            data: {
                                states: getSessionsMapFromSessions(
                                    call.sessions,
                                ),
                                channelID: data[i].channel_id,
                            },
                        ...
                        if (ringingEnabled(store.getState()) && data[i].call) {
                            ...
                            if (dismissed) {
                                ...
                                if (
                                    Object.hasOwn(dismissed, currentUserID) &&
        ...
        const registerHeaderMenuComponentIfNeeded = async (
            channelID: string,
        ) => {
            try {
        ...
        const onActivate = async (wsClient?: WebSocketClient) => {
            ...
            const currentCallChannelID = channelIDForCurrentCall(
                store.getState(),
            );
            ...
            if (currentCallChannelID) {
                if (wsClient) {
                    ...
                    wsClient.sendMessage(
                        ...
                        { channelID: currentCallChannelID },
                    );
                } else {
        ...
        let joinCallParam = new URLSearchParams(window.location.search).get(
            'join_call',
        );
        this.unsubscribers.push(
            store.subscribe(() => {
                ...
                if (currChannelId !== currentChannelId) {
                    ...
                    if (
                        currChannelId &&
                        Boolean(joinCallParam) &&
                        !channelIDForCurrentCall(store.getState())
                    ) {
        ...
        this.unsubscribers.push(() =>
            document.removeEventListener('keydown', handleKBShortcuts, true),
        );
    }
