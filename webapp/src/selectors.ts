...
const clientState = (state: GlobalState) =>
    pluginState(state).clientStateReducer;
...
export const channelForCurrentCall: (
    state: GlobalState,
) => Channel | undefined = createSelector(
...
export const threadIDForCurrentCall: (
    state: GlobalState,
...
const getProfilesMapFromSessions = (
    sessions: Record<string, UserSessionState>,
    ...
    createSelector(
        ...
        (profiles, sessions, channelID) => {
            return Object.values(
                getProfilesMapFromSessions(sessions[channelID], profiles),
            );
        },
    );

export const profilesInCallInCurrentChannel: (
    state: GlobalState,
    ...
    (profiles, sessions, currChannelId) => {
        return Object.values(
            getProfilesMapFromSessions(sessions[currChannelId], profiles),
        );
    },
);

// profilesInCurrentCallMap creates an id->UserProfile object for the currently connected call.
export const profilesInCurrentCallMap: (state: GlobalState) => {
...
export const profilesInCallInChannel = (
    state: GlobalState,
    channelID: string,
): UserProfile[] => {
    return Object.values(
        getProfilesMapFromSessions(
            sessionsInCalls(state)[channelID],
            userProfiles(state),
        ),
    );
};

export const channelHasCall = (
    state: GlobalState,
    channelId: string,
): boolean => {
...
export const sessionsInCurrentCallMap: (state: GlobalState) => {
...
export const callStartAtForCallInChannel = (
    state: GlobalState,
    channelID: string,
): number => {
...
export const callStartAtForCurrentCall: (state: GlobalState) => number =
    createSelector(
        ...
        channelIDForCurrentCall,
        getCallsClientInitTime,
        (callsStates, channelID, initTime) =>
            callsStates[channelID]?.startAt || initTime || 0,
    );

export const callInCurrentChannel: (
    state: GlobalState,
) => callState | undefined = createSelector(
...
export const idForCallInChannel = (
    state: GlobalState,
    channelID: string,
): string | undefined => {
...
export const callOwnerIDForCallInChannel = (
    state: GlobalState,
    channelID: string,
): string | undefined => {
...
export const callDismissedNotification = (
    state: GlobalState,
    channelID: string,
) => {
...
const screenSharingIDsForCalls = (
    state: GlobalState,
): screenSharingIDsState => {
    return pluginState(state).screenSharingIDs;
};

export const screenSharingSessionForCurrentCall: (
    state: GlobalState,
) => UserSessionState | undefined = createSelector(
...
export const threadIDForCallInChannel = (
    state: GlobalState,
    channelID: string,
) => {
...
export const recordingForCurrentCall: (
    state: GlobalState,
) => CallJobReduxState = createSelector(
...
export const hostControlNoticesForCalls = (
    state: GlobalState,
): hostControlNoticeState => {
    return pluginState(state).hostControlNotices;
};

export const hostControlNoticesForCurrentCall: (
    state: GlobalState,
) => HostControlNotice[] = createSelector(
...
export const liveCaptionsStateForCurrentCall: (
    state: GlobalState,
) => CallJobReduxState = createSelector(
...
export const areLiveCaptionsAvailableInCurrentCall: (
    state: GlobalState,
...
const recentlyJoinedUsersInCalls = (
    state: GlobalState,
): recentlyJoinedUsersState => {
    return pluginState(state).recentlyJoinedUsers;
};

export const recentlyJoinedUsersInCurrentCall: (
    state: GlobalState,
...
export const sortedIncomingCalls: (
    state: GlobalState,
) => IncomingCallNotification[] = createSelector(
    'sortedIncomingCalls',
    incomingCalls,
    (callsStates) => [...callsStates].sort((a, b) => b.startAt - a.startAt),
);

export const dismissedCalls = (
    state: GlobalState,
): { [callID: string]: boolean } => pluginState(state).dismissedCalls;
...
export const channelState = (
    state: GlobalState,
    channelId: string,
): ChannelState => pluginState(state).channels[channelId];

export const callsExplicitlyEnabled = (
    state: GlobalState,
    channelId: string,
): boolean => Boolean(channelState(state, channelId)?.enabled);

export const callsExplicitlyDisabled = (
    state: GlobalState,
    channelId: string,
): boolean => {
...
export const callsEnabledInCurrentChannel = (state: GlobalState): boolean => {
    ...
    return (
        callsExplicitlyEnabled(state, channelId) ||
...
export const callsShowButton = (
    state: GlobalState,
    channelId: string,
): boolean => !callsExplicitlyDisabled(state, channelId);

export const hasPermissionsToEnableCalls = (
    state: GlobalState,
    channelId: string,
): boolean => {
...
export const isCloudProfessionalOrEnterpriseOrTrial = (
    state: GlobalState,
): boolean =>
    isCloudProfessional(state) ||
    isCloudEnterprise(state) ||
    isCloudTrial(state);
...
export const callsUserPreferences = (
    state: GlobalState,
): CallsUserPreferences => pluginState(state).callsUserPreferences;

export const shouldPlayJoinUserSound = (state: GlobalState): boolean =>
    profilesInCurrentCall(state).length <
    callsUserPreferences(state).joinSoundParticipantsThreshold;
...
export const isAtLeastProfessional = (state: GlobalState): boolean => {
    ...
    const enterprise =
        sku === LicenseSkus.E20 || sku === LicenseSkus.Enterprise;
    const professional =
        sku === LicenseSkus.E10 || sku === LicenseSkus.Professional;

    return (
        enterprise ||
        professional ||
        isCloudProfessionalOrEnterpriseOrTrial(state)
    );
};

export const areHostControlsAllowed = (state: GlobalState): boolean =>
    callsConfig(state).HostControlsAllowed;

export const adminStats = (state: GlobalState) =>
    state.entities.admin.analytics;

export const getChannelUrlAndDisplayName = (
    state: GlobalState,
    channel?: Channel,
) => {
    ...
    if (channel) {
        ...
        if (isDirectChannel(channel)) {
            const otherUserID = getUserIdFromChannelName(
                currentUserID,
                channel.name,
            );
