...
export default async function slashCommandsHandler(
    ...
    switch (subCmd) {
        ...
        case 'start':
            if (subCmd === 'start') {
                if (
                    profilesInCallInChannel(store.getState(), args.channel_id)
                        .length > 0
                ) {
                    store.dispatch(
                        displayGenericErrorModal(
                            defineMessage({
                                defaultMessage: 'Unable to start call',
                            }),
                            defineMessage({
                                defaultMessage:
                                    'A call is already ongoing in the channel.',
                            }),
                        ),
                    );
                    return {};
            ...
            if (!connectedID) {
                ...
                if (!team_id) {
                    ...
                    if (!channel) {
                        const res = (await store.dispatch(
                            getChannelAction(args.channel_id),
                        )) as ActionResult;
                        channel = res.data;
                    }
                ...
                try {
                    ...
                    let msg = defineMessage({
                        defaultMessage:
                            'An internal error occurred and prevented you from joining the call. Please try again.',
                    });
                    if (e === DisabledCallsErr) {
                        msg = defineMessage({
                            defaultMessage:
                                'Calls are disabled in this channel.',
                        });
                    }
                    store.dispatch(
                        displayGenericErrorModal(
                            defineMessage({
                                defaultMessage: 'Unable to start or join call',
                            }),
                            msg,
                        ),
                    );
            ...
            store.dispatch(
                displayGenericErrorModal(
                    ...
                    defineMessage({
                        defaultMessage:
                            "You're already connected to a call in the current channel.",
                    }),
                ),
            );
            return {};
        case 'leave':
            ...
            store.dispatch(
                displayGenericErrorModal(
                    defineMessage({
                        defaultMessage: 'Unable to leave the call',
                    }),
                    defineMessage({
                        defaultMessage:
                            "You're not connected to a call in the current channel.",
                    }),
                ),
            );
            return {};
        case 'end':
            if (
                ...
                store.dispatch(
                    displayGenericErrorModal(
                        defineMessage({
                            defaultMessage: 'Unable to end the call',
                        }),
                        defineMessage({
                            defaultMessage:
                                "There's no ongoing call in the channel.",
                        }),
                    ),
                );
                return {};
            ...
            if (
                !isCurrentUserSystemAdmin(store.getState()) &&
                ...
                store.dispatch(
                    displayGenericErrorModal(
                        ...
                        defineMessage({
                            defaultMessage:
                                "You don't have permission to end the call. Please ask the call owner to end call.",
                        }),
                    ),
                );
                return {};
        ...
        case 'stats': {
            if (window.callsClient) {
                try {
                    ...
                    return {
                        message: `/call stats ${btoa(JSON.stringify(stats))}`,
                        args,
                    };
                } catch (err) {
        ...
        case 'recording': {
            if (
                fields.length < 3 ||
                (fields[2] !== 'start' && fields[2] !== 'stop')
            ) {
                break;
            }

            const startErrorTitle = defineMessage({
                defaultMessage: 'Unable to start recording',
            });
            const stopErrorTitle = defineMessage({
            ...
            const isHost =
                hostIDForCurrentCall(state) === getCurrentUserId(state);

            if (fields[2] === 'start') {
                trackEvent(
                    Telemetry.Event.StartRecording,
                    Telemetry.Source.SlashCommand,
                ...
                if (!isHost) {
                    store.dispatch(
                        displayGenericErrorModal(
                            ...
                            defineMessage({
                                defaultMessage:
                                    "You don't have permission to start a recording. Please ask the call host to start a recording.",
                            }),
                        ),
                    );
                    return {};
                ...
                if (!isHost) {
                    store.dispatch(
                        displayGenericErrorModal(
                            ...
                            defineMessage({
                                defaultMessage:
                                    "You don't have permission to stop the recording. Please ask the call host to stop the recording.",
                            }),
                        ),
                    );
                    return {};
