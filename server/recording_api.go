...
func (p *Plugin) recJobTimeoutChecker(callID, jobID string) {
	...
	if recState.StartAt == 0 {
		...
		recState.Props.Err = "failed to start recording job: timed out waiting for bot to join call1"
		...
		clientState.Err = "failed to start recording job: timed out waiting for bot to join call2"
